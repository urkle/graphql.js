function __extend() {
  const extended = {}, length = arguments.length
  let deep = false, i = 0
...
if (typeof XMLHttpRequest !== 'undefined') {
  ...
  __doRequest = function (
    ...
    const http = require('http'), https = require('https'), URL = require('url'), uri = URL.parse(url)
    const req = (uri.protocol === 'https:' ? https : http).request({
      ...
      headers: Object.assign({ 'Content-type': contentType, 'Accept': accept }, headers)
    }, function (response) {
      let str = ''
...
function __request(debug, method, url, headers, data, asJson, onRequestError, callback) {
  ...
  const cleaned = __handleMethod(method, url, data, asJson);

  if (debug) {
    console.groupCollapsed('[graphql]: '
      + method.toUpperCase() + ' ' + url + ': '
      + data.query.split(/\n/)[0].substring(0, 50) + '... with '
      + JSON.stringify(data.variables).substring(0, 50) + '...')
...
function GraphQLClient(url, options) {
  if (!(this instanceof GraphQLClient)) {
    const client = new GraphQLClient(url, options, true)
    const _lazy = client._sender
    for (const m in client) {
...
GraphQLClient.prototype.flatten = function (object) {
  ...
  for (const name in object) {
    if (object.hasOwnProperty(name)) {
      ...
      ? this.flatten(object[name], prefix + name + FRAGMENT_SEPARATOR, out)
...
GraphQLClient.prototype.fragmentPath = function (fragments, path) {
  const obj = fragments[path.replace(/\./g, FRAGMENT_SEPARATOR)]
...
GraphQLClient.prototype.collectFragments = function (query, fragments) {
  const that = this
  const fragmentRegexp = GraphQLClient.FRAGMENT_PATTERN
  const collectedFragments = []
  ;(query.match(fragmentRegexp)||[]).forEach(function (fragment) {
    const path = fragment.replace(fragmentRegexp, function (_, $m) {return $m})
    fragment = that.fragmentPath(fragments, path)
    if (fragment) {
      const pathRegexp = new RegExp(fragmentRegexp.source.replace(/\((.*)\)/, path) + '$')
      ...
      const alreadyCollectedFragments = collectedFragments.filter(function (alreadyCollected) {
...
GraphQLClient.prototype.processQuery = function (query, fragments) {
  ...
  const fragmentRegexp = GraphQLClient.FRAGMENT_PATTERN
  const collectedFragments = this.collectFragments(query, fragments)
  query = query.replace(fragmentRegexp, function (_, $m) {
    return "... " + $m.split(".").join(FRAGMENT_SEPARATOR)
  })
...
GraphQLClient.prototype.autoDeclare = function (query, variables) {
  ...
  return query.replace(GraphQLClient.AUTODECLARE_PATTERN, function () {
    ...
    for (const key in variables) {
      ...
      let keyAndType = key.split(/^(.*?)!/)
      if (keyAndType.length > 1) {
        keyAndType = keyAndType.slice(1)
        keyAndType[1] = keyAndType[1].replace(/(.*?)!$/, "$1")
      }
      const mapping = typeMap[typeof(value)]
      let mappedType = typeof(mapping) === "function" ? mapping(value) : mapping
      if (!key.match("!") && keyAndType[0].match(/_?id/i)) {
        mappedType = "ID"
      }
      let type = (keyAndType[1] || mappedType)
      if (type) {
...
GraphQLClient.prototype.parseType = function (query) {
  const match = query.trim().match(/^(query|mutation|subscription)/)
...
GraphQLClient.prototype.createSenderFunction = function (debug) {
  ...
  return function (query, originalQuery, type) {
    ...
    const caller = function (variables, requestOptions) {
      if (!requestOptions) requestOptions = {}
      if (!variables) variables = {}
      const fragmentedQuery = that.buildQuery(query, variables)
      const headers = Object.assign({}, that.options.headers, requestOptions.headers)
...
GraphQLClient.prototype.commit = function (mergeName) {
  ...
  Object.keys(this._transaction[mergeName]).forEach(function (method) {
    ...
    const subQuery = that._transaction[mergeName][method].map(function (merge) {
      const reqId = 'merge' + Math.random().toString().split('.')[1].substring(0, 6)
      resolveMap[reqId] = merge.resolver
      let query = merge.query.replace(/\$([^.,\s)]*)/g, function (_, m) {
        if (!merge.variables) {
          throw new Error('Unused variable on merge ' + mergeName + ': $' + m)
        }
        const matchingKey = Object.keys(merge.variables).filter(function (key) {
          return key === m || key.match(new RegExp('^' + m + '!'))
        })[0]
        const variable = reqId + '__' + matchingKey
...
GraphQLClient.prototype.createHelpers = function (sender) {
  ...
  function helper(query) {
    if (__isTagCall(query)) {
      ...
      const result = that.run(that.ql.apply(that, arguments))
      that.__prefix = ""
      that.__suffix = ""
      return result
    }
    const caller = sender(this.prefix + " " + query + " " + this.suffix, query.trim(), this.type)
  ...
  this.run = function (query, variables, options) {
    return sender(query, options)({})
  }
}
...
GraphQLClient.prototype.headers = function (newHeaders) {
  ...
  return Object.assign(this.options.headers, newHeaders)
}

GraphQLClient.prototype.fragment = function (fragment) {
  if (typeof fragment == 'string') {
    const _fragment = this._fragments[fragment.replace(/\./g, FRAGMENT_SEPARATOR)]
    if (!_fragment) {
...
GraphQLClient.prototype.ql = function (strings) {
  let fragments = Array.prototype.slice.call(arguments, 1)
  fragments = fragments.map(function (fragment) {
    if (typeof fragment == 'string') {
      return fragment.match(/fragment\s+(\S*)\s/)[1]
    }
  })
  let query = (typeof strings == 'string') ? strings : strings.reduce(function (acc, seg, i) {
